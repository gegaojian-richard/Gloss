...
void Model::run(string doc_pt, string mw_pt, int gen_all, string res_dir) {
  load_docs(doc_pt, mw_pt, gen_all);
...
// format: wid  wid  wid ...
void Model::load_docs(string dfile, string word_dict_file, int gen_all) {
...
//  todo: 扩充bs
 cout << "load word_dict: " << word_dict_file << endl;
 ifstream rf2( word_dict_file.c_str() );
 if (!rf2) {
	cout << "file not find:" << word_dict_file << endl;
	exit(-1);
 }

 if(gen_all == 1){
     while(getline(rf2, line)){
        Doc doc(line);
        doc.gen_biterms(bs);
     }
 }else{
